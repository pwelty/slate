...
class LinkwardenAdapter {
  ...
  getApiBaseUrl() {
    // Use the current domain with port 3001 for the API server
  ...
  async fetchRecentItems() {
    ...
    const apiUrl = `${this.getApiBaseUrl()}/linkwarden/recent`
...
class TriliumAdapter {
  ...
  async fetchRecentItems() {
    const { limit = 3 } = this.config
    const apiUrl = `${this.getApiBaseUrl()}/trilium/recent`
...
class ObsidianAdapter {
  ...
  async fetchRecentItems() {
    const { limit = 3 } = this.config
    const apiUrl = `${this.getApiBaseUrl()}/obsidian/recent`
